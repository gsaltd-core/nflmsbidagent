...
def invoke_agent(agent_id, agent_alias_id, session_id, prompt):
    try:
        client = boto3.session.Session().client(service_name="bedrock-agent-runtime", region_name='eu-west-2')               #1 Add Region Name
        ...
        response = client.invoke_agent(
            agentId='IFF1J6DPCM',                     #2 Add Agent ID
            agentAliasId='EH7VROCXV8',                #3 Add Agent Alias ID
            enableTrace=True,
